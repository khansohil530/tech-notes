...
If our queries include ANDing multiple columns for search, we can hugely benefit by creating **Composite 
Indexes**. **Composite Indexes** include more than one key column in index for searching. 
In Postgres, you can create these as follows,
```postgresql
...
- Composite Index is built from left to right in Postgres. That means the order in which columns are provided
  when creating composite index matters, the left most column can still be used by itself for filtering with
  the composite index, but it can’t be used by the rest of columns on right.
- Composite Index will be used when we’ve AND searching on key columns but if we search by ORing the key columns
  Postgres would go for a sequential scan.
...
UUID in talk are UUID4 which are completely random and no two UUID generated consecutively are nearby each other.
When you create an index organized table, the leaf nodes indicate the page/rows which are ordered on your
indexing key. When the key your using is completely random (like UUID4), you end up frequently encountering
problems like *page split*. However, if we had a sequential key for index, we’ll have no page split as
they’ll fill up sequentially from left to right and essentially minimizes page splitting.  

But how read would be impacted by this? When we fetch a row, the whole page is loaded into a shared memory
known as buffer pool. If the buffer is full, DB will eliminate the oldest page to load up the current page.
And in case of UUIDs, we’ll end up loading and removing page randomly because we’ve no ordering. 
Instead, if we had sequential key for index, we’d have related pages which loaded up into memory,
so if let’s say there's a surge in read for a category of products, they’ll be present in same nearby pages
because their id are sequential. 

???+ note "Page Split" 
    When your Index page is full of rows, and you insert an entry which belongs in b/w the page,
    you’ll have split it and reorganize and rereference everything. This is very costly and should be minimized.
