---
tags:
    - Database
---

# Fundamentals of Database Engineering

![Udemy](https://img.shields.io/badge/Platform-Udemy-purple?logo=udemy&logoColor=#A435F0)
![Completed](https://img.shields.io/badge/Status-Completed-green)

[Course Link](https://www.udemy.com/course/database-engines-crash-course/){target=_blank}

Notes:

- [x] [ACID](acid.md)
- [x] [Understanding Database Internals](db_internals.md)
- [x] [Database Indexing](indexing.md)
- [x] [B-Trees vs B+-Trees](db_index.md)
- [x] [Database Partitioning](db_partition.md)
- [x] [Database Sharding](db_shard.md)
- [x] [Concurrency Control](concurrency_control.md)
- [x] [Replication](replication.md)
- [x] [Database Engines](db_engines.md)
- [x] [Database Cursor](db_cursor.md)
- [x] [NoSQL Architecture](nosql.md)
- [x] [Database Security](db_security.md)
- [x] [Extra](extra.md)
