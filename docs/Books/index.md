---
comments: false
hide:
  - footer
  - toc
---
# DSA

<table>
    <thead>
        <tr>
            <th>Cover</th>
            <th>Title</th>
            <th>Description</th>
            <th>References</th>
        </tr>
    </thead>
    <tbody>
         <tr>
            <td><img class="customEntitityAlbum" style="background-color:transparent" src="ddia/static/ddia.png"/></td>
            <td>
                Designing Data-Intensive Applications <br/>
              <img src="https://img.shields.io/badge/inactive-orange" alt="status: completed">
            </td>
            <td>The book discusses various principles and trade-offs that are fundamental to data systems, as it 
                explores various design decisions taken by different products.</td>
            <td>
                <a target="_blank" href="ddia/">notes</a><br/>
                <a target="_blank" href="https://www.amazon.in/Designing-Data-Intensive-Applications-Reliable-Maintainable/dp/9352135245">
                    link
                </a>
        </tr>
    </tbody>
</table>